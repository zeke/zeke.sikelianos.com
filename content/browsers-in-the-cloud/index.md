...
I recently wrote about [how to drive your local Chrome browser with an AI agent](/driving-chrome-with-an-agent). In this post, I want to show you how to do the same thing with browsers running in the cloud.
...
The cloud MCP server gives your agent a clean, disposable browser session running on Cloudflare's [Browser Rendering](https://developers.cloudflare.com/browser-run/) service. It's the same `chrome-devtools-mcp` package, but pointed at a remote WebSocket endpoint instead of your local Chrome.
...
You'll need a Cloudflare account and an API token with `Browser Rendering - Edit` permission. Export them in your shell:
...
> Use the cloud browser to check the performance of https://example.com
