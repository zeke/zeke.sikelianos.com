...
const getImageColors = require('get-image-colors') // eslint-disable-line no-unused-vars
...
module.exports = async function pageImages (pagePath) {
  ...
  const images = walk(path.dirname(pagePath))
    .filter(({ relativePath }) => {
      // only images directly in the page's own directory, not nested
      // subdirectories (e.g. sibling pages' photo galleries)
      return !relativePath.includes(path.sep) && formats.includes(path.extname(relativePath))
    })
    .map(fileData => {
      const ext = path.extname(fileData.relativePath)
      const fullPath = path.join(fileData.basePath, fileData.relativePath)
      const image = {
        ...fileData,
        fullPath,
