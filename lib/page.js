...
class Page {
  constructor (opts) {
    ...
    // Only pages whose filename is index.md/index.html represent a full
    // "page" at their directory's URL (e.g. dial-a-repo/index.md -> /dial-a-repo).
    // Sibling files like dial-a-repo/transcript.md are real pages with their
    // own URLs, but they're not what people mean by "the dial-a-repo page",
    // so they're excluded from listings (homepage, sitemap, RSS) by default.
    this.isIndexPage = /(^|\/)index\.(md|html)$/.test(this.relativePath)
